  ...
  const saveDocument = useCallback(
      debounce(async (content: string, lang: string) => {
        ...
        try {
          ...
          setSaveStatus("error");
          setAuthError(error instanceof Error ? error.message : "Failed to save document");
        }
      }, 1000),
      [session, docId]
  );
      ...
      <div className="h-[600px] w-full border border-gray-300 rounded flex flex-col">
        ...
        {authError ? (
            <div className="bg-red-100 border-l-4 border-red-500 text-red-700 p-4 flex-grow">
              ...
              <p>{authError}</p>
              <p className="mt-2">Please ensure you are logged in and try refreshing the page.</p>
            </div>
        ) : (
            <MonacoEditor
                height="100%"
